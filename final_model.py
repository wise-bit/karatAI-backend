...
clean_directory()
vidcap = cv2.VideoCapture('./test/sample1.mp4') ## Insert video
...
while success:
    ...
    for index in required_points:
        meta_json["coordinates"][index+1] = {}
        
        if (len(detected_keypoints[index])):
            final_keypoints.append(detected_keypoints[index][0][0:2])
            meta_json["coordinates"][index+1]["x"] = detected_keypoints[index][0][0]
            meta_json["coordinates"][index+1]["y"] = detected_keypoints[index][0][1]
        else:
            final_keypoints.append(None)
            meta_json["coordinates"][index+1]["x"] = 0
            meta_json["coordinates"][index+1]["y"] = 0
